...
class Cifar100_with_CLIP_embedding(torch.utils.data.Dataset):
    def __init__(self, root, train, download, transform):
        super().__init__()
        self.cifar100 = torchvision.datasets.CIFAR100(root=root, train=train, download=download, transform=transform)
        if download:
            os.makedirs("clipembeddingdata", exist_ok=True)
            import requests

            # URLs for the files you want to download
            url2 = 'https://dl.dropboxusercontent.com/scl/fi/bciifiu2ap728eczu8m51/train_embeddings_tensor.pt?rlkey=sj9k3a3hm81zuqotfv2x7jv80&st=luau0e4x&dl=1'
    ...
    def __getitem__(self, idx):
        ...
        clipembedding = self.embeddings[idx]
        return (img, clipembedding), label
...
def get_training_dataloader(mean, std, batch_size=16, num_workers=2, shuffle=True,
    ...
    cifar100_training = dataset_class(root='./data', train=True, download=True, transform=transform_train)
...
def get_test_dataloader(mean, std, batch_size=16, num_workers=2, shuffle=True,
    ...
    cifar100_test = dataset_class(root='./data', train=False, download=True, transform=transform_test)
